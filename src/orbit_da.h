...
#define da_init(da_ptr, capacity) do { \
    ...
    (da_ptr)->len = 0; \
    (da_ptr)->cap = c; \
    (da_ptr)->at = dbmalloc(sizeof((da_ptr)->at[0]) * c); \
} while (0)

#define da_append(da_ptr, element) do { \
    if ((da_ptr)->len == (da_ptr)->cap) { \
        (da_ptr)->cap *= 2; \
        (da_ptr)->at = dbrealloc((da_ptr)->at, sizeof(*(da_ptr)->at) * (da_ptr)->cap); \
    } \
...
#define da_shrink(da_ptr) do { \
    ...
    (da_ptr)->at = dbrealloc((da_ptr)->at, sizeof((da_ptr)->at[0]) * (da_ptr)->cap); \
} while (0)
